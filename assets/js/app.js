...
import feather from '../libs/feather-icons/feather.min';
import SimpleBar from '../libs/simplebar/simplebar.min';
import countdown from '../libs/countdown/jquery.countdown.min.js';

const flatpickr = require('flatpickr/dist/flatpickr.min');
const Francais = require("flatpickr/dist/l10n/fr.js").default.fr;
...
$(document).ready(function () {
    ...
    $('#clock').countdown($('#countdown_delay').val()).on('update.countdown', function (event) {
        let format = '<span class="counter-number secondaire">Délai restant pour annuler : <i class="las la-clock"></i> %M : %S</span>';
    ...
    function updateGrade1() {
        const s = $("#grade1_val").val();
        let limit_for_car_max = $("#limit_for_car_max").val();
        if (s.length !== 0) {
            let g2 = 0;
            const val_input = parseInt(s);
            if ($("#grade2_val").val() !== "")
                g2 = parseInt($("#grade2_val").val());
            let sum = g2 + val_input;
            if (sum > limit_for_car_max) {
                sum = limit_for_car_max;
                const new_val = limit_for_car_max - g2;
                $("#grade1_val").val(new_val);
            }
            $("#sum_qty").val(sum);
        }
    }
    ...
    function updateGrade2() {
        const s = $("#grade2_val").val();
        let limit_for_car_max = $("#limit_for_car_max").val();
        if (s.length !== 0) {
            let g2 = 0;
            const val_input = parseInt(s);
            if ($("#grade1_val").val() !== "")
                g2 = parseInt($('#grade1_val').val());
            let sum = g2 + val_input;
            if (sum > limit_for_car_max) {
                sum = limit_for_car_max;
                const new_val = limit_for_car_max - g2;
                $("#grade2_val").val(new_val);
            }
            $("#sum_qty").val(sum);
        }
    }

    $("#grade1_val").on("input", function (e) {
        updateGrade1();
    });

    $("#grade1_val").blur(function () {
        var s = $(this).val();
        if (s.length === 0) {
            $(this).val("0");
        }
    ...
    $("#grade2_val").on("input", function (e) {
        updateGrade2();
    });

    $("#grade2_val").blur(function () {
        var s = $(this).val();
        if (s.length === 0) {
            $(this).val("0");
        }
    ...
    let region = document.querySelector("#gas_station_region");
    region.addEventListener("change", function () {
        let form = this.closest('form');
        let data = region.name + "=" + region.value;
        console.log(data);
        fetch(form.action, {
            ...
            .then((html) => {
                ...
                let nouveauSelect = content.querySelector('#gas_station_city');
                document.querySelector('#gas_station_city').replaceWith(nouveauSelect);
...
!function () {
    let a, n, o, s, t, d, e, l, r, i, m, c = document.querySelector(".navbar-menu").innerHTML, u = 7, g = "en", b = localStorage.getItem("language");
    function y(e) {
    ...
    function S(e) {
        ...
        } else if (1 === f(e.target.parentElement.nextElementSibling) && 1848 <= window.innerWidth) for (a = document.getElementsByClassName("dropdown-custom-right"); 0 < a.length;) a[0].classList.remove("dropdown-custom-right");
